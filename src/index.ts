...
export function expandMultiplyShorthand(file: NormalizedTokens): ExpandedStep {
  ...
  const expandedSteps = steps.map(({ instructions, stepNumber, endCount }) => {
    ...
    const [blockRepeat, blockMultiplier] = instructions
      .match(/(\(.*\)x\d+)/g) // matches text like (2sc, dec)x5
      ?.[0] // returns the match element
      .split('x') ?? [] // split the matched string into ['(2sc, dec)', '5']; coalecse into an array for destructuring

    if (!blockRepeat || !blockMultiplier) throw new Error(`Something went wrong when getting blockRepeat (${blockRepeat}), and blockMultiplier (${blockMultiplier})`)

    const blockRepeatClean = blockRepeat.replace('(', '').replace(')', ''); // '(2sc, dec)' => '2sc, dec'

    const expanded = new Array<string>(parseInt(blockMultiplier)) // creates array of length equal to the multiplier (e.g. 5)
      .fill(blockRepeatClean) // fills the array with the block of repeated instructions (e.g. '2sc, dec')
    ...
    const insertExpandedIntoInstructions = instructions
      ...
      .map<string[]>((token) => {
        ...
        return new Array(parseInt(miniMutliplier)).fill(miniRepeat) // effectively multiplies our stitch by the appropriate amount.
      })
      .reduce((prev, curr) => [...prev, ...curr], []) // flattens into a single array
